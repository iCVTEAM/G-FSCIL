...
def base_train(model, trainloader, optimizer, scheduler, epoch, args):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        loss_bss = 0
        # calculation bss loss
        u,s,v = torch.svd(logits.t())
        ll = s.size(0)
        for t in range(1):
            loss_bss = loss_bss + torch.pow(s[ll-1-t],2)
        
        # end of calculation on BSS loss
        acc = count_acc(logits, train_label)

        total_loss = loss_ce + loss_bss*0.001
        # the bss loss can be omitted
