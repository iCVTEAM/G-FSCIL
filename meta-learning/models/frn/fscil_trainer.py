#from models.frn.fscil_trainer import FSCILTrainer as Trainer
...
class FSCILTrainer(Trainer):
    ...
    def get_optimizer_base_unused(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                for epoch in range(args.epochs_base):
                    ...
                    # train base sess
                    self.model.train() # this additional/ no
                    tl, ta = self.base_train_meta(self.model, trainloader, optimizer, scheduler, epoch, args)
                    self.model.eval() 
                    #self.model = construct_protos(train_set, testloader.dataset.transform, self.model, args)
                    ...
                    if args.set_no_val: # set no validation
                        ...
                        self.best_model_dict = deepcopy(self.model.state_dict())
                        tsl, tsa, ts_base, ts_new  = test(self.model, testloader, epoch, args, session)
                        ...
                        print('epoch:%03d,lr:%.4f,training_base:%.5f,training_base:%.5f' % (
                            epoch, lrc, ts_base, ts_new))
                        ...
                        result_list.append(
                            'epoch:%03d,training_base:%.5f,training_new:%.5f' % (
                                epoch, ts_base, ts_new))
                    elif epoch % 10 == 0 :
                        #and (epoch>0)
                        # take the last session's testloader for validation
                        model = construct_protos(train_set, testloader.dataset.transform, self.model, args)

                        vl, va, ts_base, ts_new = self.validation(model, epoch, args)

                        # save better model
                        if (va * 100) >= self.trlog['max_acc'][session]:
                            self.trlog['max_acc'][session] = float('%.3f' % (va * 100))
                            self.trlog['base_acc'][session] = float('%.3f' % (ts_base * 100))
                            self.trlog['new_acc'][session] = float('%.3f' % (ts_new * 100))
                ...
                print('The base acc={:.3f}'.format(self.trlog['base_acc'][session]))
                print('The new acc={:.3f}'.format(self.trlog['new_acc'][session]))

                result_list.append('Session {}, Test Best Epoch {},\nbest test Acc {:.4f}\n'.format(
                    session, self.trlog['max_acc_epoch'], self.trlog['max_acc'][session], ))
                result_list.append('Session {}, Test Best Epoch {},\nbase Acc {:.4f}, new Acc {:.4f}\n'.format(
                    session, self.trlog['max_acc_epoch'], self.trlog['base_acc'][session], self.trlog['new_acc'][session], ))
                ...
                self.model.module.frn_finetune(trainloader, trainloader.dataset.transform, testloader.dataset.transform, np.unique(train_set.targets), session)
                epoch = 0
                tsl, tsa, ts_base, ts_new = test(self.model, testloader, epoch, args, session)
    ...
    def validation(self, model, epoch, args):
        ...
        with torch.no_grad():
            ...
            for session in range(1, self.args.sessions):
                ...
                model.eval()
                #trainloader.dataset.transform = testloader.dataset.transform
                model.module.frn_finetune(trainloader, trainloader.dataset.transform, testloader.dataset.transform, np.unique(train_set.targets), session)

                #model.module.update_prototypes(trainloader, np.unique(train_set.targets), session)
                vl, va, ts_base, ts_new = test(model, testloader, epoch, self.args, session)

        return vl, va, ts_base, ts_new

    def base_train_meta(self, model, trainloader, optimizer, scheduler, epoch, args):
        tl = Averager()
        ta = Averager()
        ...
        for i, batch in enumerate(tqdm_gen, 1):
            ...
            '''
            data = model(data, 'feature')
            # N,HW,C
            resolution = data.size(1)
            # k = args.episode_way * args.episode_shot
            proto, query = data[:k], data[k:]

            # featuremap/query: way*query_shot, resolution, d
            # support: way, shot*resolution , d              
            proto = proto.view(args.episode_shot, args.episode_way, resolution, proto.shape[-1])
            query = query.view(args.episode_query*args.episode_way, resolution, query.shape[-1])

            proto = proto.mean(0)
           


            logits = model.module.proto_measure(feature_map=query, support_proto=proto, current_class_num=args.episode_way)
    ...
    def test_old(self, model, testloader, args, session):
        ...
        with torch.no_grad():
            for i, batch in enumerate(testloader, 1):
                ...
                proto = model.module.proto.weight[:test_class, :].detach()
