...
class MYNET(nn.Module):
    ...
    def get_recon_dist(self, query, support, alpha, beta, verbose=False):
        ...
        Woodbury =True
        if  Woodbury:
            support_square = support_trans.matmul(support)
            m_inv = (support_square+torch.eye(support_square.size(-1)).to(support_square.device).unsqueeze(0).mul(lam)).inverse() # way, d, d
            hat = m_inv.matmul(support_square) # way, d, d
            query_proj = query.matmul(hat).mul(rho)
        else:
    ...
    def forward(self, x, mode = 'feature',  target=None):
        # target is label
    ...
    def proto_measure(self, feature_map, support_proto, current_class_num):
        # featuremap/query: way*query_shot, resolution, d
        # support: way, resolution , d  
        # target is label
        batch_size = feature_map.size(0)
    ...
    def update_proto_ft(self, new_proto, dataloader, label, session):
        ...
        for key, value in self.encoder.named_parameters():
            ...
            if "bn" in key:
                ...
                pass
                #optimized_parameters += [{"params": [value]}]
        ...
        #                            weight_decay=0)
        optimizer = torch.optim.Adam(optimized_parameters, lr=self.args.lr_new, weight_decay=5e-4)
        current_class_num = self.args.base_class + self.args.way * (session)

        softmax_func = nn.Softmax(dim=1)

        with torch.enable_grad():
            #base_proto = self.proto.data[:self.args.base_class, :].detach()
            prev_class_num = self.args.base_class + self.args.way * (session - 1)
            ...
            for epoch in tqdm(range(self.args.epochs_new)):
                for i, batch in enumerate(dataloader):
                    ...
                    ## constructing random sampling feature for generalization 
                    #flatten_embedding  = embedding.view(embedding.size(0),-1)
                    flatten_embedding  = embedding.view(-1)
                    mean_response  = torch.mean(flatten_embedding) # size of N
                    std_response  = torch.std(flatten_embedding)  # size of N
                    
                    max_response  = torch.max(flatten_embedding) # size of N
                    min_response  = torch.min(flatten_embedding)  # size of N                    
                    # generating
                    sample_embedding = torch.normal(mean_response,std_response,size=(embedding.size(0), embedding.size(1), embedding.size(2))).cuda()
                    #sample_embedding = torch.randn(embedding.size(0), embedding.size(1), embedding.size(2)).uniform_(min_response,max_response).cuda()
                    sample_logits = self.proto_measure(sample_embedding, old_proto, prev_class_num)  # this should be further modified initialized with only base
                    
                    sample_logits = sample_logits-torch.min(sample_logits)
                    sample_logits = softmax_func(sample_logits)
                    #print(sample_embedding[0,:])
                    #norm_logits = sample_logits/torch.sum(sample_logits, dim=1)
                    #norm_logits = torch.norm(sample_logits,p=1,dim=1)
                    onehot = F.one_hot(label,num_classes=current_class_num)
                    softlabel = onehot.clone().float()
                    
                    #lam = np.random.beta(0.2, 0.2)
                    lam = torch.rand(embedding.size(0),1)*0
                    lam = 1-lam
                    lam_label = lam.clone().repeat(1,current_class_num).cuda()
                    lam_embedding = lam.clone().unsqueeze(-1).repeat(1,embedding.size(1),embedding.size(2)).cuda()
                    #softlabel[:, :prev_class_num] = softlabel[:, :prev_class_num] *lam_label[:, :prev_class_num] + (1-lam_label[:, :prev_class_num])*sample_logits
                    #softlabel[:, prev_class_num:] = softlabel[:, prev_class_num:] *lam_label[:, prev_class_num:]
                    #print(softlabel[0,:])
                    soft_embedding = embedding*lam_embedding + (1-lam_embedding)*sample_embedding
                    ## end of construction
                    logits = self.proto_measure(soft_embedding, proto,current_class_num)
                    loss = self.smoothCE(logits, softlabel)
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
                    print('epoch:%03d,lr:%.4f,training_loss:%.5f' % (epoch, self.args.lr_new, loss.item()))
    ...
    def smoothCE(self, inputs, targets):
        """
        Args:
        ...
        log_probs = logsoftmax_func(inputs)
